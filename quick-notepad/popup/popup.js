...
var note=document.getElementById('note'),count=document.getElementById('count');
chrome.storage.local.get('note',function(d){if(d.note)note.value=d.note;updateCount();});
note.addEventListener('input',function(){chrome.storage.local.set({note:note.value});updateCount();});
document.getElementById('clearBtn').onclick=function(){note.value='';chrome.storage.local.set({note:''});updateCount();};
function updateCount(){var w=note.value.trim()?note.value.trim().split(/\s+/).length:0;count.textContent=w+' words, '+note.value.length+' chars';}
