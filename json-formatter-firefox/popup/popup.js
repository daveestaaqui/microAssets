...
(function() {
  ...
  _api.storage.local.get([PRO_KEY, PRO_BOUGHT_KEY, 'json_formatter_and_viewer_pro_usage'], (r) => {
    if (r[PRO_BOUGHT_KEY] || r[PRO_KEY]) return;
    const usage = r['json_formatter_and_viewer_pro_usage'] || 0;
    if (usage >= 10) {
...
(function() {
  const _api = (typeof browser !== 'undefined' && browser.storage) ? browser : chrome;
  const EXT_ID = 'json_formatter_and_viewer_pro';
...
toggle.addEventListener('change', () => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0]) {
      chrome.tabs.sendMessage(tabs[0].id, { type: 'TOGGLE', enabled: toggle.checked })
        .catch(() => {}); // ignore if content script not injected on this tab
    }
  });
});

// Dynamic Review Link
document.getElementById('reviewLink')?.addEventListener('click', (e) => {
  e.preventDefault();
  chrome.management.getSelf((self) => {
    window.open(`https://chrome.google.com/webstore/detail/${self.id}/reviews`);
  });
});
