...
(function() {
  ...
  _api.storage.local.get([PRO_KEY, PRO_BOUGHT_KEY, 'dark_mode_for_all_websites_usage'], (r) => {
    if (r[PRO_BOUGHT_KEY] || r[PRO_KEY]) return;
    const usage = r['dark_mode_for_all_websites_usage'] || 0;
    if (usage >= 10) {
...
(function() {
  const _api = (typeof browser !== 'undefined' && browser.storage) ? browser : chrome;
  const EXT_ID = 'dark_mode_for_all_websites';
