...
(function() {
  ...
  const EXT_ID = 'json_to_csv_converter';
...
document.getElementById('convertBtn').onclick=()=>{try{const d=JSON.parse(document.getElementById('input').value);if(!Array.isArray(d))throw new Error('Input must be a JSON array');const keys=Object.keys(d[0]);const csv=[keys.join(','),...d.map(r=>keys.map(k=>{const v=String(r[k]||'');return v.includes(',')?'"'+v+'"':v;}).join(','))].join('\n');document.getElementById('output').value=csv;}catch(e){document.getElementById('output').value='Error: '+e.message;}};
