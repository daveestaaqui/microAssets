...
(function() {
  ...
  const EXT_ID = 'word_and_character_counter_for_any_page';
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        return { words: words.length, chars, sentences, paragraphs, readingTime };
      }
    }, (injectionResults) => {
      if (injectionResults && injectionResults[0]) {
        const r = injectionResults[0].result;
        document.getElementById('wordCount').textContent = r.words.toLocaleString();
        document.getElementById('charCount').textContent = r.chars.toLocaleString();
        document.getElementById('sentenceCount').textContent = r.sentences.toLocaleString();
        document.getElementById('paraCount').textContent = r.paragraphs.toLocaleString();
        document.getElementById('readTime').textContent = r.readingTime + ' min';
