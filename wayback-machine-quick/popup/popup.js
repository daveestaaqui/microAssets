...
(function() {
  ...
  const EXT_ID = 'wayback_machine_quick';
...
document.addEventListener('DOMContentLoaded', () => {
  chrome.tabs.query({active: true, currentWindow: true}, tabs => {
    ...
    document.getElementById('open').onclick = () => chrome.tabs.create({url: 'https://web.archive.org/web/*/' + url});
    document.getElementById('latest').onclick = () => chrome.tabs.create({url: 'https://web.archive.org/web/2/' + url});
    document.getElementById('save').onclick = () => chrome.tabs.create({url: 'https://web.archive.org/save/' + url});
  });
});