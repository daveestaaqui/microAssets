...
(function() {
  ...
  const EXT_ID = 'amazon_fake_review_skimmer';
  ...
  _api.storage.local.get([USAGE_KEY], (r) => {
    ...
    if (count >= 3) { const fb = document.getElementById('ma-feedback'); if (fb) fb.style.display = 'block'; }
  });
...
chrome.storage.local.get([STORAGE_KEY], (r) => { toggle.checked = r[STORAGE_KEY] !== false; });
...
document.getElementById('reviewLink')?.addEventListener('click', (e) => {
  e.preventDefault();
  chrome.management.getSelf((self) => { window.open('https://chrome.google.com/webstore/detail/' + self.id + '/reviews'); });
});
