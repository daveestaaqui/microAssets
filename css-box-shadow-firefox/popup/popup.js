...
(function() {
  ...
  const EXT_ID = 'css_box_shadow_generator';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const inputs = ['x','y','blur','spread'].map(id => document.getElementById(id));
  const color = document.getElementById('color');
  
  function update() {
    const shadow = inputs[0].value + 'px ' + inputs[1].value + 'px ' + inputs[2].value + 'px ' + inputs[3].value + 'px ' + color.value;
